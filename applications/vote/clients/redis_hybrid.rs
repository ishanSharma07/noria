    ...
    fn call(&mut self, req: ReadRequest) -> Self::Future {
        ...
        async move {
            ...
            if !misses.is_empty() {
                ...
                let (_, rows): (_, Vec<(i32, String, i32)>) = qresult.collect_and_drop().await.unwrap();
                ...
                ServiceExt::<Vec<(i32, String, i32)>>::ready_oneshot(r_conn)
                    .await?
                    .call(rows)
                    .await?;
            }
...
impl Service<Vec<(i32, String, i32)>> for RedisConn {
    type Response = ();
    type Error = failure::Error;
    type Future = impl Future<Output = Result<(), failure::Error>> + Send;
    ...
    fn call(&mut self, req: Vec<(i32, String, i32)>) -> Self::Future {
        let mut conn = self.c.clone();
        let mut batch = redis::pipe();
        for (aid, mut article_title, vote_count) in req {
            // Construct the value.
            article_title.push_str(", ");
            article_title.push_str(&vote_count.to_string());
            batch.cmd("SET").arg(aid).arg(article_title).ignore();
        }
