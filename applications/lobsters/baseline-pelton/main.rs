...
const BASELINE_SCHEMA: &'static str = include_str!("db-schema/pelton.sql");
        ...
        if priming {
            ...
            Box::pin(async move {
                ...
                let schema = match variant {
                    Variant::BaselinePelton => BASELINE_SCHEMA,
                };
        ...
        Box::pin(async move {
            let inner = async move {
                let (c, with_notifications) = match variant {
                    Variant::BaselinePelton => handle_req!(pelton, req),
                }?;
                ...
                if let Some(uid) = acting_as {
                    if with_notifications && !priming {
                        match variant {
                            Variant::BaselinePelton => endpoints::pelton::notifications(c, uid).await,
                        }?;
                    }
                }
